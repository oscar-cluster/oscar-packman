...
# Set the ChRoot instance variable for this object. A value of undef
# is treated as a directive to quash all chrooted tags, ostensibly
# operating on the real root filesystem.
sub chroot {
...
# treated as a directive to quash all cache tags, ostensibly operating
...
# bit of boilerplate for completely handling the #chroot and
# guaranteeing certain properties of the #args tags in *_command_line
# returned strings.  Also breaks off the command name for separate
# handling.
sub command_helper {
  ...
  if (defined ($self->{Cache})) {
    ...
    my $cache_arg = $self->cache_arg_command_line;

    if ($cache_arg =~ m/#cache/) {
      # put everywhere #cache tag is
      $cache_arg =~ s/#cache/$self->{Cache}/g;
    } else {
      # put on end
...
sub do_complex_command {
  ...
  # If the command is aggregatable, we can run one command for the
  # whole shameel.
  ...
  if ($aggregatable) {
    ...
    if ($pid) {
	  ...
	  if( $items[0] eq "OK" ) {
	      push @dependencies, $items[2];
          } else {
	      push @captured_output, $items[1] unless $items[2];
          }
      }
      close (SYSTEM);

      # Check the exit status (in $? after close)

      my $status = $? >> 8;
      if ($status) {
        my $msg = join( "\n ", @captured_output );
        die "DepMan: $command failed to select the following packages:\n $msg\n\n"
      }
    } else {
  ...
  # Otherwise, we have to loop over and run the command once for each
  # item in the list of victims.
	...
	my $status = $? >> 8;
	if ($status == 0) {
	    push @dependencies, @captured_output;
	} else {
	    die "DepMan: $command failed: $!";
	}
      } else {
	my $line = $cl;
...
# Query the underlying dependency manager to report the list of which
# of the package files in the dependency database are required/relied
# by any of the package files in the argument list.
...
# Query the underlying dependency manager to report the list of which
# of the currently installed packages are required/relied on by any of
# the currently installed packages in the argument list.
sub query_requires {
...
=head1 ABSTRACT
  ...
  DepMan is essentially an abstract class, even though Perl doesn't
  have them. It's expected there will be additional modules under
  DepMan:: to handle concrete dependency managers while DepMan itself
  acts as the front-door API.

=head1 DESCRIPTION

  There are two basic methods on DepMan objects. They are both queries
  which return lists suitable for passing to PackMan->install() or
  PackMan->remove().

  DepMan->query_requires() takes the names of packages, presumably
  already installed, and returns the list of packages, also already
  installed, which require the packages in the arguments. Semanticly,
  replace "query" with the word "what". DepMan->query_requires
  ('glibc') is the same as asking, "DepMan, what requires glibc?" (if
  you actually try this, don't be surprised when you get the phone
  book as output).

  DepMan->query_required_by() takes the names of package files,
  presumably not yet installed, and returns the list of package files,
  also not yet installed, which will be required to be installed
  before the package files in the arguments can be installed. Similar
  to requires, DepMan->query_required_by ('glibc') is the same as
  asking, "DepMan, what is required by glibc?"

  The return values are guaranteed to have the names of all valid
  arguments.  Any invalid arguments are silently omitted.
